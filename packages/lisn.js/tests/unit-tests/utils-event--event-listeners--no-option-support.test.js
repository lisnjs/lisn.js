...
let MOCKED_LISTENER_OPTIONS = 0;
Element.prototype.addEventListener = (eventType, handler, options) => {
  if (eventType === "testOptionSupport") {
    MOCKED_LISTENER_OPTIONS = 1;
    Element.prototype.addEventListener = originalAddEventListener; // just once
    throw "options should be a boolean";
  } else {
...
afterAll(() => {
  expect(MOCKED_LISTENER_OPTIONS).toBe(1);
});
