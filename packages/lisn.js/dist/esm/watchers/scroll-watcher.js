...
import { scrollTo, getCurrentScrollAction, getClientWidthNow, getClientHeightNow, tryGetMainScrollableElement, fetchMainContentElement, fetchMainScrollableElement, fetchScrollableElement, isValidScrollDirection } from "../utils/scroll.js";
...
const hasExceededThreshold = (options, latestData, lastThresholdData) => {
  ...
  const topDiff = maxAbs(latestData[MC.S_SCROLL_TOP] - lastThresholdData[MC.S_SCROLL_TOP], latestData[MC.S_SCROLL_HEIGHT] - lastThresholdData[MC.S_SCROLL_HEIGHT], latestData[MC.S_CLIENT_HEIGHT] - lastThresholdData[MC.S_CLIENT_HEIGHT]);
  const leftDiff = maxAbs(latestData[MC.S_SCROLL_LEFT] - lastThresholdData[MC.S_SCROLL_LEFT], latestData[MC.S_SCROLL_WIDTH] - lastThresholdData[MC.S_SCROLL_WIDTH], latestData[MC.S_CLIENT_WIDTH] - lastThresholdData[MC.S_CLIENT_WIDTH]);
...
const invokeCallback = (callback, element, scrollData) => callback.invoke(element, MH.copyObject(scrollData)).catch(logError);
