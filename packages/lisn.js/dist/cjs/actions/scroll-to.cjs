...
class ScrollTo {
  ...
  constructor(element, config) {
    ...
    this.undo = async () => {
      if (prevScrollTop !== -1) {
        const action = await watcher.scrollTo({
          top: prevScrollTop,
          left: prevScrollLeft
        }, (0, _misc.omitKeys)(config !== null && config !== void 0 ? config : {}, {
          offset: true
    ...
    this[MC.S_TOGGLE] = async () => {
      ...
      const action = await watcher.scrollTo(element, MH.merge(config, canReverse ? {
        altTarget
...
const newConfigValidator = element => {
  return {
    ...
    duration: (key, value) => (0, _validation.validateNumber)(key, value),
