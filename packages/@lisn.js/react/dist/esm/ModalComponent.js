const _excluded = ["as", "children", "triggers", "config", "widgetRef"];
...
import { createElement, useRef, useEffect, useImperativeHandle } from "react";
import { Modal } from "lisn.js";
export const ModalComponent = _ref => {
  let {
      as,
      children,
      triggers,
  ...
  useEffect(() => {
    const triggerElements = triggers === null || triggers === void 0 ? void 0 : triggers.map(r => r.current).filter(e => e !== null);
    if (elementRef.current instanceof HTMLElement) {
      widgetRefInternal.current = new Modal(elementRef.current, Object.assign({
