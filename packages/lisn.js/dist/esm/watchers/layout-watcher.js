 ...
 */
export var LayoutWatcher = /*#__PURE__*/function () {
  function LayoutWatcher(config, key) {
                ...
                var intersectionHandler = function intersectionHandler(entries) {
                  ...
                  if (!isReady) {
                    ...
                    if (numEntries < NUM_LAYOUTS) {
                      logWarn(MH.bugError("Got IntersectionObserver ".concat(numEntries, ", ") + "expected >= ".concat(NUM_LAYOUTS)));
                    }
                  }
    ...
    var processLayoutChange = function processLayoutChange(skipCallbacks) {
      var deviceBit = MH.floor(MH.log2(nonIntersectingBitmask & ORDERED_DEVICES.bitmask));
      var aspectRatioBit = MH.floor(MH.log2(nonIntersectingBitmask & ORDERED_ASPECTR.bitmask));
      var layoutData = {
        device: null,
        aspectRatio: null
      };
...
var getConfig = function getConfig(config) {
  var deviceBreakpoints = MH.copyObject(settings.deviceBreakpoints);
  if (config !== null && config !== void 0 && config.deviceBreakpoints) {
    copyExistingKeys(config.deviceBreakpoints, deviceBreakpoints);
  }
  var aspectRatioBreakpoints = MH.copyObject(settings.aspectRatioBreakpoints);
...
var getNonIntersecting = function getNonIntersecting(nonIntersectingBitmask, entry) {
  ...
  if (!MH.isHTMLElement(target)) {
    logError(MH.bugError("IntersectionObserver called us with '".concat(MH.typeOrClassOf(target), "'")));
  ...
  if (!layout) {
    ...
    /* istanbul ignore next */ // shouldn't happen
    logError(MH.bugError("Unknown device or aspectRatio data attribute: ".concat(layout)));
  }
...
var invokeCallback = function invokeCallback(callback, layoutData) {
  return callback.invoke(MH.copyObject(layoutData))["catch"](logError);
};
