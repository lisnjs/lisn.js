...
const getWheelGestureFragment = (events, options) => {
  ...
  for (const event of events) {
    ...
    const data = (0, _normalizeWheel.normalizeWheel)(event);
    let thisIntent = MC.S_SCROLL;
    let thisDeltaX = data.pixelX;
    let thisDeltaY = data.pixelY;
    let thisDeltaZ = 1;
    const maxDelta = (0, _math.havingMaxAbs)(thisDeltaX, thisDeltaY);
...
exports.getWheelGestureFragment = getWheelGestureFragment;
