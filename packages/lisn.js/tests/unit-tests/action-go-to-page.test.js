...
const { GoToPage, fetchAction } = window.LISN.actions;
...
test("basic", async () => {
  ...
  const action = new GoToPage(element, 3);
...
test("is registered", async () => {
  ...
  const action = await fetchAction(element, "go-to-page", "3");
  expect(action).toBeInstanceOf(GoToPage);
