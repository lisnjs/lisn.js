...
class ViewWatcher {
  ...
  constructor(config, key) {
    ...
    const xObserver = new _xIntersectionObserver.XIntersectionObserver(intersectionHandler, observeOptions);
    ...
    const setupOnView = async (target, handler, userOptions, trackType) => {
      ...
      if (viewData.rootBounds[MC.S_WIDTH] === 0 && viewData.rootBounds[MC.S_HEIGHT] === 0) {
        ...
        await (0, _domOptimize.waitForSubsequentMeasureTime)();
    ...
    const setupInviewTrack = async (options, viewCallback, viewData) => {
      ...
      const addTrackCallback = () => {
        var _config$_root;
        const trackCallback = (0, _callback.wrapCallback)(async () => {
...
const fetchOptions = async (root, target, options) => {
  return {
    ...
    _viewsBitmask: (0, _views.getViewsBitmask)(options === null || options === void 0 ? void 0 : options.views),
...
const viewChanged = (latestData, prevData) => !prevData || viewsToBitmask(prevData.views) !== viewsToBitmask(latestData.views) || !(0, _misc.compareValuesIn)(MH.copyBoundingRectProps(prevData.targetBounds), MH.copyBoundingRectProps(latestData.targetBounds)) || !(0, _misc.compareValuesIn)(prevData.rootBounds, latestData.rootBounds) || !(0, _misc.compareValuesIn)(prevData.relative, latestData.relative);
const viewsToBitmask = views => _views.VIEWS_SPACE.bit[views[0]] | (views[1] ? _views.VIEWS_SPACE.bit[views[1]] : 0);
const fetchIntersectionData = async (config, entryOrTarget, realtime = false) => {
  const root = config._root;
  const vpSize = await (0, _size.fetchViewportSize)(realtime);
  const rootMargins = (0, _text.toMargins)(config._rootMargin, vpSize);
  let target;
  let targetBounds;
  let rootBounds = null;
...
const fetchBounds = async (root, realtime, rootMargins) => {
  ...
  if (root) {
    ...
    const {
      width,
      height
    } = await (0, _size.fetchViewportSize)(realtime);
    rect = {
      x: 0,
      left: 0,
      right: width,
...
const fetchViews = async (intersection, realtime, useScrollingAncestor) => {
  ...
  if (!intersection._isCrossOrigin) {
    ...
    const scrollingAncestor = (0, _scroll.getClosestScrollable)(useScrollingAncestor !== null && useScrollingAncestor !== void 0 ? useScrollingAncestor : intersection._target);
...
const fetchElement = async (root, target) => {
  ...
  return await (0, _overlays.createOverlay)(overlayOptions);
};
const getOverlayOptions = (root, target) => {
  const {
    reference,
    value
  } = (0, _views.parseScrollOffset)(target);
  let ovrDimension;
...
const invokeCallback = (callback, element, viewData) => callback.invoke(element, MH.copyObject(viewData)).catch(_log.logError);
