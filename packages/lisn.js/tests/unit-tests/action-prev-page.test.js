...
const { PrevPage, fetchAction } = window.LISN.actions;
...
test("basic", async () => {
  ...
  const action = new PrevPage(element);
...
test("is registered", async () => {
  ...
  const action = await fetchAction(element, "prev-page");
  expect(action).toBeInstanceOf(PrevPage);
