 ...
 */
export const registerOpenable = (name, newOpenable, configValidator) => {
 ...
 */
export class Openable extends Widget {
  /**
...
export class Popup extends Openable {
  ...
  constructor(element, config) {
    ...
    if (container && position === S_AUTO) {
      // Automatic position
      this.onOpen(async () => {
        const [contentSize, containerView] = await MH.promiseAll([SizeWatcher.reuse().fetchCurrentSize(element), ViewWatcher.reuse().fetchCurrentView(container)]);
...
const getTriggersFrom = (content, inputTriggers, wrapTriggers, prefixedNames) => {
  ...
  const addTrigger = (trigger, triggerConfig) => {
    if (wrapTriggers) {
      const wrapper = MH.createElement(isInlineTag(MH.tagName(trigger)) ? "span" : "div");
      wrapElement(trigger, {
        wrapper,
        ignoreMove: true
      }); // no need to await
...
const setupListeners = (widget, elements, properties, prefixedNames) => {
  ...
  const isTrigger = element => MH.includes(MH.arrayFrom(triggers.keys()), element.closest(getDefaultWidgetSelector(prefixedNames._trigger)));
