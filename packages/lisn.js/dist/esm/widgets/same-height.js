...
export class SameHeight extends Widget {
  ...
  constructor(containerElement, config) {
    ...
    this.toColumn = () => setData(containerElement, MC.PREFIX_ORIENTATION, MC.S_VERTICAL);
...
const isText = element => getData(element, PREFIX_ITEM__FOR_SELECT) === S_TEXT || getData(element, PREFIX_ITEM__FOR_SELECT) !== S_IMAGE && MH.isHTMLElement(element) && MH.lengthOf(element.innerText) >= MIN_CHARS_FOR_TEXT;
...
const fetchConfig = async (containerElement, userConfig) => {
  ...
  const minGap = getNumValue(MH.strReplace(colGapStr, /px$/, ""), settings.sameHeightMinGap);
  return {
...
const findItems = containerElement => {
  const items = [...MH.querySelectorAll(containerElement, getDefaultWidgetSelector(PREFIX_ITEM__FOR_SELECT))];
