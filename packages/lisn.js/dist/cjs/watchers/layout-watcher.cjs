...
class LayoutWatcher {
  ...
  constructor(config, key) {
        ...
        const intersectionHandler = entries => {
          ...
          if (!isReady) {
            ...
            if (numEntries < _layout.NUM_LAYOUTS) {
              (0, _log.logWarn)(MH.bugError(`Got IntersectionObserver ${numEntries}, ` + `expected >= ${_layout.NUM_LAYOUTS}`));
            }
          }
    ...
    const setupOnLayout = async (handler, options) => {
      const layoutBitmask = (0, _layout.getLayoutBitmask)(options);
    ...
    const processLayoutChange = skipCallbacks => {
      const deviceBit = MH.floor(MH.log2(nonIntersectingBitmask & _layout.ORDERED_DEVICES.bitmask));
      const aspectRatioBit = MH.floor(MH.log2(nonIntersectingBitmask & _layout.ORDERED_ASPECTR.bitmask));
      const layoutData = {
        device: null,
        aspectRatio: null
      };
...
const getConfig = config => {
  ...
  const aspectRatioBreakpoints = MH.copyObject(_settings.settings.aspectRatioBreakpoints);
...
const createOverlays = async (root, deviceBreakpoints, aspectRatioBreakpoints) => {
  ...
  if (root) {
    ...
    overlayParent = await (0, _overlays.createOverlay)({
      style: {
        position: "fixed",
        [MC.S_WIDTH]: "100vw"
      }
...
const getOverlayLayout = overlay => {
  const layout = (0, _cssAlter.getData)(overlay, PREFIX_DEVICE) || (0, _cssAlter.getData)(overlay, PREFIX_ASPECTR);
...
const getNonIntersecting = (nonIntersectingBitmask, entry) => {
  ...
  if (!MH.isHTMLElement(target)) {
    (0, _log.logError)(MH.bugError(`IntersectionObserver called us with '${MH.typeOrClassOf(target)}'`));
  ...
  if (!layout) {
    ...
    /* istanbul ignore next */ // shouldn't happen
    (0, _log.logError)(MH.bugError(`Unknown device or aspectRatio data attribute: ${layout}`));
  }
...
const invokeCallback = (callback, layoutData) => callback.invoke(MH.copyObject(layoutData)).catch(_log.logError);
